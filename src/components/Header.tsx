      ...
      <div className="mx-auto max-w-[1280px] px-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3 sm:gap-6 h-[64px]">
          ...
          <div className="flex-1 max-w-[560px] relative hidden md:block">
            ...
            <Input value={search} onChange={e=>setSearch(e.target.value)} onFocus={onSearchFocus} placeholder="Describe what you lost..." className="pl-9 pr-10 h-10 rounded-full bg-muted/50 border-0 focus-visible:ring-1" />
          ...
          <div className="hidden sm:flex items-center gap-2">
            <Select value={locationFilter} onValueChange={setLocationFilter}>
              ...
              <SelectContent><SelectItem value="all">All VIT Chennai Locations</SelectItem>{VIT_LOCATIONS.map(l=> <SelectItem key={l.id} value={l.id}>{l.label}</SelectItem>)}</SelectContent>
            </Select>
          </div>
        ...
        <div className="md:hidden pb-3 flex gap-2">
          <div className="flex-1 relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search lost items..." className="pl-9 h-10 rounded-full bg-muted/50 border-0" /></div>
          <Select value={locationFilter} onValueChange={setLocationFilter}><SelectTrigger className="w-[44px] h-10 rounded-full bg-muted/50 border-0 p-0 justify-center"><MapPin className="h-4 w-4"/></SelectTrigger><SelectContent><SelectItem value="all">All locations</SelectItem>{VIT_LOCATIONS.map(l=> <SelectItem key={l.id} value={l.id}>{l.label}</SelectItem>)}</SelectContent></Select>
        </div>
      </div>
    </header>
  )
