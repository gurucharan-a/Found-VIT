...
export async function analyzeImages(opts: {
  ...
  onProgress?.("Analyzing images with Gemini…")
...
export function scoreMatch(query: string, post: { title: string; description: string; analysis?: GeminiAnalysis; locationLabel: string }): number {
  ...
  const haystack = [post.title, post.description, post.analysis?.category, post.analysis?.item, post.analysis?.color, post.analysis?.brand, ...(post.analysis?.characteristics ?? []), post.locationLabel].join(" ").toLowerCase()
